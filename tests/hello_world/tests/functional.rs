...
fn setup(program_id: &Pubkey) -> ProgramTest {
    let mut program_test = ProgramTest::new(
        "hello_world",
        *program_id,
        processor!(hello_world::__process_instruction),
    );
...
async fn success_execute() {
    ...
    let mint_address = hello_world::MINT;
    ...
    let rent_lamports = rent.minimum_balance(
        ExtraAccountMetaList::size_of(hello_world::MyExtraMetas::to_extra_account_metas().len())
            .unwrap(),
    );
...
async fn fail_incorrect_derivation() {
    ...
    // wrong derivation
    let extra_account_metas_address = get_extra_account_metas_address(&program_id, &mint_address);
...
async fn fail_incorrect_mint() {
    ...
    let token_program_id = spl_token_2022::id();
    let wallet = Keypair::new();
    let mint_address = Pubkey::new_unique(); // wrong mint
    let mint_authority = Keypair::new();
    let mint_authority_pubkey = mint_authority.pubkey();
    let source = Pubkey::new_unique();
    let destination = Pubkey::new_unique();
...
#[tokio::test]
async fn fail_incorrect_program_id() {
    let program_id = Pubkey::new_unique(); // wrong program id
    let mut program_test = setup(&program_id);
...
async fn success_on_chain_invoke() {
    let hook_program_id = hello_world::ID;
...
//     let token_program_id = spl_token_2022::id();
//     let wallet = Keypair::new();
//     let mint_address = ahoy_world::MINT;
//     let mint_authority = Keypair::new();
//     let mint_authority_pubkey = mint_authority.pubkey();
//     let source = Pubkey::new_unique();
//     let destination = Pubkey::new_unique();
